...
# (b) For extra analysis, compute a Taylor series approximation at the mid-point t=0.5 (for demonstration)
taylor_points = []
for t in t_values:
    taylor_points.append(taylor_approximation(0.5, t, capitals["USA"], capitals["Russia"], A=2.0, B=3.0, k=3, phase=0))
...
plt.title("Battlefront Curve and Taylor Approximation with Deployment Points")
...
# For USA deployment, true μ = 160; for Russia, true μ = 30.
sim_results = []
for i, d in enumerate(deployment_data):
    ...
    if d["country"] == "USA":
        req_est = math.ceil((haversine(capitals["USA"][0], capitals["USA"][1], d["lat"], d["lon"]) / final_est) * defense_specs["USA"]["cost_factor"])
    else:
        req_est = math.ceil((haversine(capitals["Russia"][0], capitals["Russia"][1], d["lat"], d["lon"]) / final_est) * defense_specs["Russia"]["cost_factor"])
    sim_results.append({
        "point_index": i,
        "country": d["country"],
...
plt.title("Monte Carlo Simulation of Cost-Efficient Weapon Requirement")
