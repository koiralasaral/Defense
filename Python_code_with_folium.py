...
def adjust_speed_for_range(rocket_name_adj, target_range_km, initial_launch_angle_deg=45):
    ...
    for iteration in range(max_iterations):
        ...
        if best_achieved_range_km == -1 or abs(achieved_range_km - target_range_km) < abs(
                best_achieved_range_km - target_range_km):
...
def convert_local_meters_to_global_lat_lon_alt(local_x_meters, local_y_meters, altitude_meters, launch_lat_deg,
                                               launch_lon_deg):
    """
...
satellite_plot, = ax.plot([sat_x_init], [sat_y_init], [sat_z_init], 's', color='cyan', markersize=8,
                          label='Satellite')  # 's' for square marker
...
for rocket_name, params in missiles_to_simulate.items():
    ...
    for i in range(len(x_traj_2d)):
        ...
        lat, lon, alt = convert_local_meters_to_global_lat_lon_alt(local_x_disp, local_y_disp, y_traj_2d[i],
    ...
    for layer_name, layer_params in ATMOSPHERE_LAYERS.items():
        ...
        for idx in entry_indices:
            ...
            if y_traj_2d[idx + 1] - y_traj_2d[idx] != 0:
                ...
                coll_global_lat, coll_global_lon, coll_global_alt = convert_local_meters_to_global_lat_lon_alt(
                    local_x_disp, local_y_disp, coll_local_y_alt, launch_lat_deg, launch_lon_deg)
                # Convert to global XYZ
                coll_global_X, coll_global_Y, coll_global_Z = convert_lat_lon_alt_to_cartesian(coll_global_lat,
                                                                                               coll_global_lon,
                                                                                               coll_global_alt)
                ...
                collision_points_for_rocket.append((coll_global_X, coll_global_Y, coll_global_Z, f'Entry {layer_name}',
                                                    coll_global_lat, coll_global_lon, coll_global_alt, flux_value,
                                                    velocity_vector_at_collision.tolist(),
                                                    normal_vector_at_collision.tolist()))
        ...
        for idx in exit_indices:
            if y_traj_2d[idx + 1] - y_traj_2d[idx] != 0:
                ...
                collision_points_for_rocket.append((coll_global_X, coll_global_Y, coll_global_Z, f'Exit {layer_name}',
                                                    coll_global_lat, coll_global_lon, coll_global_alt, None, None,
                                                    None))

    collision_markers[rocket_name] = \
    ax.plot([], [], [], 'X', color=params["color"], markersize=8, label=f'{rocket_name} Collisions')[0]
    ...
    impact_x, impact_y, impact_z = trajectory_data[rocket_name]['x_global'][-1], \
    trajectory_data[rocket_name]['y_global'][-1], trajectory_data[rocket_name]['z_global'][-1]
    ...
    crater_plots[rocket_name] = ax.plot_surface(crater_X, crater_Y, crater_Z, color='brown', alpha=0.6,
                                                label=f'{rocket_name} Crater')
...
def update_animation(frame):
    ...
    for rocket_name, data in trajectory_data.items():
        ...
        trajectory_lines[rocket_name].set_data_3d(data['x_global'][:current_idx + 1],
                                                  data['y_global'][:current_idx + 1],
                                                  data['z_global'][:current_idx + 1])
        artists.append(trajectory_lines[rocket_name])

        # Update rocket point (use global coordinates)
        trajectory_points[rocket_name].set_data_3d([data['x_global'][current_idx]], [data['y_global'][current_idx]],
                                                   [data['z_global'][current_idx]])
        ...
        for coll_global_x, coll_global_y, coll_global_z, coll_type, coll_global_lat, coll_global_lon, coll_global_alt, flux_val, vel_vec, norm_vec in \
        data['collisions']:
            # Check if the rocket has passed the collision point in terms of distance from launch
            current_rocket_pos = np.array(
                [data['x_global'][current_idx], data['y_global'][current_idx], data['z_global'][current_idx]])
            collision_point_pos = np.array([coll_global_x, coll_global_y, coll_global_z])

            if np.linalg.norm(
                    current_rocket_pos - convert_lat_lon_alt_to_cartesian(NEPAL_LAT_CENTER, NEPAL_LON_CENTER, 0)) > \
                    np.linalg.norm(
                        collision_point_pos - convert_lat_lon_alt_to_cartesian(NEPAL_LAT_CENTER, NEPAL_LON_CENTER, 0)):
                ...
                if "Entry" in coll_type and flux_val is not None and vel_vec is not None and norm_vec is not None:
                    ...
                    v_arrow = ax.quiver(coll_global_x, coll_global_y, coll_global_z,
                                        ...
                                        color='lime', length=ARROW_LENGTH_SCALE / 2, arrow_length_ratio=0.3,
                    ...
                    n_arrow = ax.quiver(coll_global_x, coll_global_y, coll_global_z,
                                        ...
                                        color='blue', length=ARROW_LENGTH_SCALE / 4, arrow_length_ratio=0.3,
                                        label='Normal Vector')
...
all_final_x_local = [data['final_impact_x_local'] for data in trajectory_data.values() if
                     'final_impact_x_local' in data]
all_final_y_local = [data['final_impact_y_local'] for data in trajectory_data.values() if
                     'final_impact_y_local' in data]
...
circle = plt.Circle((target_mu[0], target_mu[1]), TARGET_RADIUS_M, color='blue', fill=False, linestyle='--',
                    label=f'{TARGET_AREA_KM2} km\u00b2 Target Area')
...
def visualize_with_pydeck(trajectory_data_pydeck, collision_markers_data_pydeck, mcmc_results_pydeck,
                          target_radius_pydeck):
    ...
    for rocket_name, data in trajectory_data_pydeck.items():
        ...
        for i in range(len(data['x_global'])):
            lat, lon, alt = convert_cartesian_to_lat_lon_alt(data['x_global'][i], data['y_global'][i],
                                                             data['z_global'][i])
        ...
        for coll_global_x, coll_global_y, coll_global_z, coll_type, coll_global_lat, coll_global_lon, coll_global_alt, flux_val, vel_vec, norm_vec in collisions:
            ...
            if "Entry" in coll_type and flux_val is not None and vel_vec is not None and norm_vec is not None:
                ...
                all_velocity_vectors_pydeck.append({
                    ...
                    'target_position': [coll_global_lon + vel_vec[0] / ARROW_LENGTH_SCALE,
                                        coll_global_lat + vel_vec[1] / ARROW_LENGTH_SCALE,
                                        coll_global_alt + vel_vec[2] / ARROW_LENGTH_SCALE],
                ...
                all_normal_vectors_pydeck.append({
                    'source_position': [coll_global_lon, coll_global_lat, coll_global_alt],
                    'target_position': [coll_global_lon + norm_vec[0] / ARROW_LENGTH_SCALE,
                                        coll_global_lat + norm_vec[1] / ARROW_LENGTH_SCALE,
                                        coll_global_alt + norm_vec[2] / ARROW_LENGTH_SCALE],  # Scale vector
                ...
                all_flux_markers_pydeck.append({
                    'position': [coll_global_lon, coll_global_lat, coll_global_alt + flux_radius],
                    # Place slightly above collision marker
                    'radius': flux_radius,
    ...
    sat_lat_pydeck, sat_lon_pydeck, sat_alt_pydeck = convert_local_meters_to_global_lat_lon_alt(0, 0, satellite_alt_m,
                                                                                                sat_lat_init,
                                                                                                sat_lon_init)
    ...
    for sample_x_local, sample_y_local in mcmc_results_pydeck['samples_array']:
        ...
        lat, lon, _ = convert_local_meters_to_global_lat_lon_alt(relative_x, relative_y, 0, NEPAL_LAT_CENTER,
                                                                 NEPAL_LON_CENTER)
    ...
    print(
        "PyDeck visualization saved to nepal_rocket_simulation_pydeck.html. Please open this file in a web browser to view the interactive map.")
...
def visualize_with_folium(trajectory_data_folium, mcmc_samples_folium, target_mu_folium, target_radius_folium,
                          failure_rate_percent_folium):
    ...
    folium.Marker(
        ...
        icon=folium.DivIcon(
            html=f'<div style="font-size: 12pt; color: black; background-color: white; padding: 5px; border-radius: 5px;">Failure Rate: {failure_rate_percent_folium:.2f}%</div>')
    ).add_to(m_satellite)
...
for rocket_name, data in trajectory_data.items():
    if rocket_name in trajectory_data:
        folium_trajectory_data[rocket_name] = {
            ...
            'global_alts': data['global_alts'],
            # Altitude is not directly used by Folium PolyLine for 3D, but good to have
            'color': data['color'],
