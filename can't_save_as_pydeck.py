...
ax.set_title('Rocket Trajectories through Atmospheric Layers over Nepal (Matplotlib)')
ax.legend()
...
def convert_meters_to_lat_lon(x_meters, y_meters):
    """Converts meters from Nepal's origin to approximate latitude and longitude."""
...
def visualize_with_pydeck(trajectory_data_pydeck, collision_markers_data_pydeck):
    ...
    for rocket_name, data in trajectory_data_pydeck.items():
        # Convert trajectory points (x_shifted, y_shifted, z) to lat/lon/alt
        path_coords = []
        for i in range(len(data['x'])):
            lat, lon = convert_meters_to_lat_lon(data['x'][i], data['y'][i])
            path_coords.append([lon, lat, data['z'][i]])
    ...
    for rocket_name, collisions in collision_markers_data_pydeck.items():
        for coll_x, coll_y, coll_z, coll_type in collisions:
            lat, lon = convert_meters_to_lat_lon(coll_x, coll_y)
            all_collision_points.append({
                'position': [lon, lat, coll_z],
