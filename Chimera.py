...
for target_rocket in rockets:
    ...
    if len(coeffs) > 1:
        ...
        b_coeff = float(coeffs[1].split(" + ")[0])
    ...
    for other_rocket in rockets:
        if other_rocket != target_rocket:
            ...
            if len(other_coeffs) > 1:
                ...
                other_b_coeff = float(other_coeffs[1].split(" + ")[0])
...
# 3D Blast Damage Visualization for Chimera
def plot_blast_damage_area(energy_joules, material_strength=1e7):
    """
    Plots the blast damage area based on energy and material strength.

    Args:
        energy_joules (float): Energy released in Joules.
        material_strength (float): Material strength in Pascals.
    """
    radius_km = calculate_crater_radius_from_energy(energy_joules, material_strength)
    ...
    ax.set_title(f"Blast Damage Area\nRadius: {radius_km:.2f} km")
    ax.set_xlabel("X (km)")
    ax.set_ylabel("Y (km)")
    ax.set_zlabel("Z (km)")
    plt.show()
def plot_3d_blast_damage(energy_joules, title="3D Blast Damage Area"):
    """
    Visualizes the blast damage area in 3D based on the energy released.

    Args:
        energy_joules (float): Energy released in Joules.
        title (str): Title of the plot.
    """
    # Simplified model: Assume a spherical blast area.  In reality, the shape is much more complex.
    radius_m = (energy_joules / 1e7)**(1/3)  # Very simplified radius calculation
