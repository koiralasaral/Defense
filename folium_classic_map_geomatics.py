...
for rocket_name, params in missiles_to_simulate.items():
    ...
    for _ in range(3): # Plot 3 arrows per rocket
        v_arrow = ax.quiver([], [], [], [], [], [], color='green', length=50000, arrow_length_ratio=0.3)
        a_arrow = ax.quiver([], [], [], [], [], [], color='purple', length=50000, arrow_length_ratio=0.3)
    ...
    for layer_name, layer_params in ATMOSPHERE_LAYERS.items():
        ...
        for idx in entry_indices:
            ...
            if y_traj_2d[idx+1] - y_traj_2d[idx] != 0:
                ...
                collision_points_for_rocket.append((coll_global_X, coll_global_Y, coll_global_Z, f'Entry {layer_name}', coll_global_lat, coll_global_lon, coll_global_alt))
        ...
        for idx in exit_indices:
            if y_traj_2d[idx+1] - y_traj_2d[idx] != 0:
                ...
                # Convert to global XYZ
                coll_global_X, coll_global_Y, coll_global_Z = convert_lat_lon_alt_to_cartesian(coll_global_lat, coll_global_lon, coll_global_alt)
                collision_points_for_rocket.append((coll_global_X, coll_global_Y, coll_global_Z, f'Exit {layer_name}', coll_global_lat, coll_global_lon, coll_global_alt))
...
def init_animation():
    for rocket_name in missiles_to_simulate:
        if rocket_name in trajectory_lines:
            ...
            for arrow in velocity_arrows[rocket_name]:
                arrow.remove() # Remove old arrows
    ...
    return list(trajectory_lines.values()) + list(trajectory_points.values()) + list(collision_markers.values()) + list(crater_plots.values()) + [satellite_plot]
...
def update_animation(frame):
    ...
    for rocket_name, data in trajectory_data.items():
        ...
        coll_y_display = []
        coll_z_display = []
        for coll_global_x, coll_global_y, coll_global_z, _, _, _, _ in data['collisions']: # Unpack all elements
        ...
        for idx in arrow_indices:
            ...
            if v_mag > 0:
                ...
                v_scaled_direction = np.array(v_direction) / v_mag * 50000 # Scale by 50km
            ...
            if a_mag > 0:
                # Scale arrow length for visibility
                a_scaled_direction = np.array(a_direction) / a_mag * 50000 # Scale by 50km
...
def visualize_with_pydeck(trajectory_data_pydeck, collision_markers_data_pydeck, mcmc_results_pydeck, target_radius_pydeck):
    ...
    for rocket_name, collisions in collision_markers_data_pydeck.items():
        for coll_global_x, coll_global_y, coll_global_z, coll_type, coll_global_lat, coll_global_lon, coll_global_alt in collisions:
            # These are already global lat/lon/alt, so use them directly.
...
def visualize_with_folium(trajectory_data_folium, mcmc_samples_folium, target_mu_folium, target_radius_folium, failure_rate_percent_folium):
    ...
    for rocket_name, data in trajectory_data_folium.items():
        ...
        for coll_global_x, coll_global_y, coll_global_z, coll_type, coll_global_lat, coll_global_lon, coll_global_alt in data['collisions']:
            folium.CircleMarker(
                ...
                fill=True,
                fill_color='red',
                tooltip=f"{rocket_name} {coll_type} at {coll_global_alt/1000:.2f} km"
            ).add_to(m_satellite)
...
# Main execution flow (kept as is, but now calls Folium at the end)
